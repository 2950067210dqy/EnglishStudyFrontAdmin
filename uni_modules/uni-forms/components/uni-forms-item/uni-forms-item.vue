<template>
	<view class="uni-forms-item"
		...
		<slot name="label">
			<view class="uni-forms-item__label" style="height: auto;" :class="{'no-label':!label && !isRequired}"
				:style="{width:localLabelWidth,justifyContent: localLabelAlign,}">
				<text v-if="isRequired" class="is-required">*</text>
				<uni-icons v-if="icon" :type="icon" :color="iconColor" size="24"></uni-icons>
				<text :style="{'margin-bottom':'20rpx','display':'block','marginLeft':'10rpx','fontSize':labelFontSize ,'color':labelColor}">{{label}}</text>
			</view >
		</slot>
		<!-- #ifndef APP-NVUE -->
		...
		methods: {
			...
			async onFieldChange(value, formtrigger = true) {
				...
				if (validateTrigger === 'bind' || formtrigger) {
					...
					if (!isRequiredField && (value === undefined )) {
						result = null;
					}
